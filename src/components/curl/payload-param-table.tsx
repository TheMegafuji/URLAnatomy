...
function ParamTableRow({
  ...
  const handleGenerate = useCallback(() => {
    ...
    const paramWithOriginalKind: AnalyzedParam = {
      ...param,
      kind: kindToUse,
    };
    const generated = generateValue(paramWithOriginalKind);
    onReplaceParam(index, generated);
  }, [index, param, kindToUse, onReplaceParam]);
...
export function PayloadParamTable({
  params,
  ...
  return (
    <>
      <table className="param-table-responsive w-full table-auto">
        ...
        <tbody>
          {params.map((param, i) => {
            if (param.kind === 'json' && param.meta) {
              const jsonMeta = param.meta as { parsed: unknown } | null;
